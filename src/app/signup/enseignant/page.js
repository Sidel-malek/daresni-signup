...
export default function SignUpPage_one (){
  ...
  <div>
    {files.map((file, index) => (
      <div key={index} className="grid grid-cols-2 slideRight"> {/* Add key here */}
        <div className="flex">
          <PaperclipIcon size={24} />
          <div className="w-72 h-10 text-sm ml-2 overflow-hidden whitespace-nowrap text-ellipsis">
            {file.name}
          </div>
        </div>
        <div className="flex justify-end">
          <button onClick={(e) => fileRemove(e, index)}>
    ...
    <div className={`relative size-24 mt-3 mb-3 ${image ? 'slideRightPhoto bg-[#FAFAFF]':' border-2 border-dashed hover:opacity-65 border-gray-300 '}   bg-gray-100 text-gray-400 cursor-pointer`}>
      ...
      <input
        ...
        onChange={imageChange}/>
              ...
              <div className="flex  ">
              <Button  className="h-8 hover:ring-1  w-full rounded-sm bg-purple-added">Continuer</Button>
              <div className="w-2 "></div>
              </div>              
